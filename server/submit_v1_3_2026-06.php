...
if ($source === 'react-page') {
    ...
    $additional_thoughts = isset($_POST['additional_thoughts']) ? htmlspecialchars(strip_tags(trim($_POST['additional_thoughts']))) : '';
...
<div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;padding:2rem;background:#ffffff;">
  ...
  ' . ($additional_thoughts ? '<div style="background:#f9f9f9;padding:1rem;border-left:3px solid #FCD116;margin-bottom:1rem;"><p style="margin:0 0 0.5rem;font-weight:bold;color:#000;">Anything else on their mind</p><p style="margin:0;color:#333;">' . nl2br($additional_thoughts) . '</p></div>' : '') . '
...
';
    ...
    if ($http_status === 200 || $http_status === 201) {
        ...
        if ($fh) {
            if ($is_new) {
                fputcsv($fh, ['Timestamp', 'Name', 'Email', 'Location', 'Document Read', 'Would Share', 'What Resonated', 'Questions Raised', 'Additional Thoughts']);
            }
            fputcsv($fh, [
                date('Y-m-d H:i:s T'),
                $name,
